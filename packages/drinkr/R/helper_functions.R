calc_widmark_factor <- function(height, weight, sex) {
  ...
  if (sex == "female") {
    ...
    # take the mean if sex is unspecified
    (r_male + r_female) / 2
  }
}

cumalative_absorption <-
  function(drinks,
           absorption_halflife,
           start_time,
           end_time) {
    ...
    t_sec <-
      seq(from = start_time,
          length.out = absorption_minutes,
          by = 60)
    # A matrix to hold the amount of alcohol absorbed from each drink at different points in time
    absorption_mat <-
      matrix(0, nrow = max(nrow(drinks), 1), ncol = absorption_minutes)
    for (i in seq_len(nrow(drinks))) {
      #The absorption equation from p. 35 in Posey and Mozayani 2007
      absorption_mat[i, ] <-
        drinks$alc_kg[i] * (1 - exp(-(t_sec - drinks$time[i]) * log(2) / absorption_halflife))
    }
    absorption_mat[absorption_mat < 0] <-
      0 # We don't absorb a negative amount of alcohol...
...
calc_bac_ts <-
  function(drinks,
    ...
    drinks$alc_kg <-
      drinks$alc_vol * 0.789 # 0.789 is the weight of one liter of alcohol
    ...
    bac_ts <-
      cumalative_absorption(drinks, absorption_halflife, start_time, end_time)
    bac_ts$time <-
      as.POSIXct(bac_ts$time, origin = "1970-01-01", tz = "UTC")

    bac_ts$bac_excluding_elimination <-
      bac_ts$kg_absorbed / (r * weight)
    bac_ts$eliminated <- rep(0, nrow(bac_ts))
    for (i in 2:nrow(bac_ts)) {
      current_bac <-
        bac_ts$bac_excluding_elimination[i] - bac_ts$eliminated[i - 1]
...
plot_bac_ts <- function(bac_ts, drinks, time_now, drink_info) {
  ...
  plot(
    ...
    bac_ts$bac_perc,
    col = rgb(0, 0, 0, 0),
    ylim = c(0, max(bac_ts$bac_perc, 0.1)),
    bty = "L",
    yaxs = "i",
    xlab = "",
    ylab = "% Blood Alcohol Concentration"
  )
  with(bac_ts[bac_ts$time <= time_now, ], lines(time, bac_perc, col = "skyblue", lwd =
                                                  4))
  with(bac_ts[bac_ts$time > time_now, ], lines(
    time,
    bac_perc,
    col = "skyblue",
    lwd = 4,
  ...
  if (time_now >= min(bac_ts$time) & time_now <= max(bac_ts$time)) {
    ...
    text(
      bac_ts$time[curr_i],
      bac_ts$bac_perc[curr_i],
      labels = paste(round(bac_ts$bac_perc[curr_i], 3), "%"),
      lwd = 4,
      pos = 4
    )
  }
  ...
  if (nrow(drinks) > 0) {
    ...
    plot(
      ...
      pch = 19,
      ylim = c(-2.5, 0.5),
      col = drink_color[drinks$name],
      xlab = "",
      ylab = "",
      xlim = range(bac_ts$time)
    )
    text(
      drinks$time,
      drink_y_pos,
