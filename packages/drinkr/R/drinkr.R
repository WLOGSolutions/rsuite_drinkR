#' @export
drinkR <- function(data_path, port, host) {
  app <- shinyApp(ui = drinkr_ui(), server = drinkr_srvr(data_path))
  ...
  shinyUI(fluidPage(
    ...
    fluidRow(
      column(
        ...
        helpText(
          "The time in minutes it takes to absorb half of the alcohol of a drink.",
        ...
        helpText(
          "The amount of % BAC you eliminate each hour.",
      ...
      column(
        ...
        fluidRow(column(
          ...
          helpText(
            ...
            br(),
            HTML(
              'Coded by Rasmus Bååth (2014), licenced under the <a rel="license" href="https://github.com/rasmusab/drinkr/blob/master/LICENCE.txt">MIT License</a>.'
            ),
            HTML(
              "Find more info about this app <a href='http://www.sumsar.net/blog/2014/07/estimate-your-bac-using-drinkr/'>on my blog</a>."
            ),
            br(),
            HTML(
              "This app is intended for entertainment purposes and might be <b>extremely missleading</b>. Never use it for any serious purpose, please!"
            )
          )
        ))
      )
    ...
    HTML(
      '<input type="text" id="client_time_zone_offset" name="client_time_zone_offset" style="display: none;"> '
    ),
...
drinkr_srvr <- function(data_path) {
  shinyServer(function(input, output, session) {
    drink_info <- read.csv(data_path, stringsAsFactors = FALSE)
    drinks <-
      data.frame(
        name = character(0),
        vol = numeric(0),
    ...
    client_start_time <- reactive({
      ...
      as.POSIXct(client_time_UTC - time_zone_offset(),
                 origin = "1970-01-01",
                 tz = "UTC")
    })

    time_zone_offset <-
      reactive(as.numeric(input$client_time_zone_offset) * 60) # in s
    ...
    output$bac_plot <- renderPlot({
      ...
      beta <-
        input$elimination / 100 / (60 * 60) # bac / sec, converted to proportion from percentage and from hour to second
      time_now <-
        as.POSIXct(as.numeric(Sys.time()),
                   origin = "1970-01-01",
                   tz = "UTC") - time_zone_offset()
    ...
    output$drink_time_input <- renderUI({
      ...
      rounded_time <-
        client_start_time() + 10 * 60 - as.numeric(client_start_time()) %% (10 * 60)
      drink_times <-
        rounded_time + seq(-12 * 60 * 60, 12 * 60 * 60, by = 10 * 60)
      time_names <- format(drink_times, format = "%H:%M")
      drink_times <- as.numeric(drink_times)
      names(drink_times) <- time_names
      selectInput("drink_time",
                  "Drink time",
                  drink_times,
                  selected = as.numeric(rounded_time))
    })

    output$drink_type_input <- renderUI({
      selectInput("drink_type",
                  "Drink type",
                  drink_info$drink,
                  selected = drink_info$drink[1])
    })
    ...
    observe({
      ...
      if (!is.null(drink)) {
        ...
        updateSliderInput(session,
                          "alc_perc",
                          value = round(drink_info$alc_prop[drink_info$drink == drink] * 100))
      }
    })
    ...
    observe({
      ...
      if (!is.null(drink) & !is.null(input$alc_perc)) {
        drink_info$alc_prop[drink_info$drink == drink] <<-
          input$alc_perc / 100
      }
    })
    ...
    output$alc_perc_input <- renderUI({
      sliderInput(
        ...
        min = 0,
        max = 100,
        value = drink_info$alc_prop[1] * 100
      )
    })
    ...
    observe({
      ...
      isolate({
        ...
        alc_prop <-
          as.numeric(input$alc_perc) / 100 # from % to prop
    ...
    output$drunken_drinks_input <- renderUI({
      if (nrow(drinks) > 0) {
        drink_labels <-
          paste(
            format(
              as.POSIXct(drinks$time, origin = "1970-01-01", tz = "UTC"),
              tz = "UTC",
              format = "%H:%M"
            ),
            " ",
            drinks$name,
            " (",
            round(drinks$vol * 100),
            " cl, ",
            round(drinks$alc_prop * 100),
            "%)",
            sep = ""
          )
    ...
    observe({
      ...
      isolate({
        ...
        if (!is.null(input$remove_drink) &&
            input$remove_drink > 0 &&
            !is.null(drink_to_remove) &&
            drink_to_remove %in% seq_len(nrow(drinks))) {
